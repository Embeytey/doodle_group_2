    ...
    def has_object_permission(self, request, view, obj):
        ...
        if view.action == "retrieve":
            ...
            try:
                ...
                schedule_poll_link = SchedulePollLink.objects.get(
...
class VotePermissions(BasePermission):
    def has_permission(self, request, view):
        ...
        schedule_poll_id = request.data.get(
        ...
        try:
            ...
            schedule_poll_link_query = Q(
                schedule_poll_link_query | Q(schedule_poll=schedule_poll)
            )
        except SchedulePoll.DoesNotExist:
            pass

        schedule_poll_link_query = (
            Q(schedule_poll=schedule_poll)
            if schedule_poll is not None
            else Q(token=link_token)
        )
